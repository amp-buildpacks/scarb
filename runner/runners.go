...
//go:generate mockery --name ScarbService --case underscore

type ScarbService interface {
	...
	CleanScarbHomeCache() error
	ScarbVersion() (string, error)
}
...
// Option is a function for configuring a ScarbRunner
type Option func(runner ScarbRunner) ScarbRunner

// WithScarbHome sets CARGO_HOME
func WithScarbHome(cargoHome string) Option {
	return func(runner ScarbRunner) ScarbRunner {
		runner.ScarbHome = cargoHome
		return runner
	}
}

// WithScarbWorkspaceMembers sets a comma separate list of workspace members
func WithScarbWorkspaceMembers(cargoWorkspaceMembers string) Option {
	return func(runner ScarbRunner) ScarbRunner {
		runner.ScarbWorkspaceMembers = cargoWorkspaceMembers
		return runner
	}
}

// WithScarbInstallArgs sets addition args to pass to cargo install
func WithScarbInstallArgs(installArgs string) Option {
	return func(runner ScarbRunner) ScarbRunner {
		runner.ScarbInstallArgs = installArgs
		return runner
	}
}
...
// ScarbRunner can execute cargo via CLI
type ScarbRunner struct {
	ScarbHome             string
	ScarbWorkspaceMembers string
	ScarbInstallArgs      string
...
// NewScarbRunner creates a new cargo runner with the given options
func NewScarbRunner(options ...Option) ScarbRunner {
	runner := ScarbRunner{}
...
func (c ScarbRunner) Install(srcDir string, destLayer libcnb.Layer) error {
...
func (c ScarbRunner) InstallMember(memberPath string, srcDir string, destLayer libcnb.Layer) error {
...
func (c ScarbRunner) InstallTool(name string, additionalArgs []string) error {
...
func (c ScarbRunner) WorkspaceMembers(srcDir string, destLayer libcnb.Layer) ([]url.URL, error) {
	m, err := c.fetchScarbMetadata(srcDir)
	if err != nil {
...
func (c ScarbRunner) ProjectTargets(srcDir string) ([]string, error) {
	m, err := c.fetchScarbMetadata(srcDir)
	if err != nil {
			...
			if pkg.ID == workspace {
				for _, target := range pkg.Targets {
					for _, kind := range target.Kind {
						if kind == "lib" && strings.HasPrefix(target.SrcPath, srcDir) {
...
// CleanScarbHomeCache clears out unnecessary files from under $CARGO_HOME
func (c ScarbRunner) CleanScarbHomeCache() error {
	files, err := os.ReadDir(c.ScarbHome)
	if err != nil {
	...
	for _, file := range files {
		...
		err := os.RemoveAll(filepath.Join(c.ScarbHome, file.Name()))
		if err != nil {
			return fmt.Errorf("unable to remove files\n%w", err)
		}
	}

	registryDir := filepath.Join(c.ScarbHome, "registry")
	...
	gitDir := filepath.Join(c.ScarbHome, "git")
...
// ScarbVersion returns the version of cargo installed
func (c ScarbRunner) ScarbVersion() (string, error) {
	buf := &bytes.Buffer{}
...
func (c ScarbRunner) BuildArgs(destLayer libcnb.Layer, defaultMemberPath string) ([]string, error) {
	envArgs, err := FilterInstallArgs(c.ScarbInstallArgs)
	if err != nil {
...
func (c ScarbRunner) fetchScarbMetadata(srcDir string) (metadata, error) {
	stdout := bytes.Buffer{}
	stderr := bytes.Buffer{}
	...
	if err := json.Unmarshal(stdout.Bytes(), &m); err != nil {
		return metadata{}, fmt.Errorf("unable to parse Scarb metadata: %w", err)
	}

	return m, nil
}

func (c ScarbRunner) makeFilterMap() map[string]bool {
	filter := c.ScarbWorkspaceMembers != ""
	filterMap := make(map[string]bool)
	if filter {
		if !strings.Contains(c.ScarbWorkspaceMembers, ",") {
			filterMap[c.ScarbWorkspaceMembers] = true
		}
		for _, f := range strings.Split(c.ScarbWorkspaceMembers, ",") {
